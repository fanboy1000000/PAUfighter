...
| F-008 | Reviewers are told not to write files rather than being unable to. Spawning them without edit tools removes the accidental edit — a reviewer helpfully fixing what it just found — but shell access remains, so the deliberate route is closed by instruction alone. | WARNING | test integrity | `docs/AGENTS.md` :: `Reviewer rules — absolute` | Closing review (5), Sprint 1; registered Sprint 2 | `open` 2026-08-31 — **Done when:** the reviewer agent cannot write a file through any route, proven by a control that attempts a shell write from a reviewer and fails. The mechanism is a permission deny-rule on write-capable shell commands. Registered in Sprint 2 because the registration floor takes every WARNING and this had been sitting in the roadmap alone, outside the sprint-close re-read. Roadmap pickup row: `B-001`. |
| F-009 | Nothing has ever confirmed the dice produce identical numbers in a browser and in Node. `vite.config.ts` sets `environment: "node"`, so the entire suite runs in one engine and structurally cannot make the cross-engine claim that invariant I1 rests on. | WARNING | I1 | `vite.config.ts` :: `test.environment`; `src/core/rng.test.ts` :: `GOLDEN_SEED_42` | Sprint 1 acceptance criterion 15, unmet; registered Sprint 2 | `open` 2026-08-31 — **Done when:** the first ten `next()` values of `createRng(42)` are produced in a browser console and match `GOLDEN_SEED_42` exactly, with the match evidenced in the pull request that closes it. Two minutes of work; needs a human with a browser. Registered in Sprint 2 for the same reason as `F-008`. Roadmap pickup row: `B-002`. |
